...
par_labels=[par_labels_count, par_labels_theta]
par_names=[par_names_count, par_names_theta] 
...
def all(DS_path):
    for path in DS_path:
        subprocess.Popen(["python3", "get_noEDM_LongitudinalField.py", "--hdf", path])
...
def time_scan(DS_path, times, key_scan):
    ...
    for time in times:
         subprocess.call(["python3", "get_noEDM_LongitudinalField.py", "--scan", key_scan, str(time)])
         # timeMod.sleep(5)
...
def both_scan(DS_path, p_min, p_max):
    ...
    for i, mom in enumerate(p_min):
        subprocess.call(["python3", "get_noEDM_LongitudinalField.py", "--scan", "--p_min", str(p_min[i]), "--p_max",  str(p_max[i])])
        # timeMod.sleep(5)
...
def plot(direction="start", bidir=False, second_direction=None):
    ...
    for i_key, key in enumerate(keys):
        ...
        for i_par in range(len(par_names[i_key])):
            ...
            # if(args.plot_p_minp_max ): ax.set_xlim(0, 3200);
            ...
            if(par_names[i_key][i_par]=='A_Bz'): ax.set_ylabel(par_labels[i_key][i_par]+" [mrad]", fontsize=font_size);
