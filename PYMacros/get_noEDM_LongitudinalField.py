...
par_names_theta= ["A_Bz", "c"]; par_labels_theta= [r"$A_{B_{z}}$", r"$c$"]; par_units_theta=[r"$\rm{\mu}$rad", r"$\rm{\mu}$rad"]
...
def plot_counts_theta(data):
    ...
    for i_station, station in enumerate(stations):
        ...
        if(not args.count):
            ...
            #Fit
            par, par_e, pcov, chi2_ndf, ndf = cu.fit_and_chi2(x, y, y_e, cu.Bz_only_phase, p0_theta_blinded[i_station])
            if (np.max(abs(par_e)) == np.Infinity ): raise Exception("\nOne of the fit parameters is infinity! Exiting...\n")
            if(args.corr): print("Covariance matrix\n", pcov); np.save("../DATA/misc/pcov_theta_S"+str(station)+".np", pcov);

            #Plot
            fig, ax, leg_data, leg_fit = cu.plot_edm(x, y, y_e, cu.Bz_only_phase, 
                                         par, par_e, chi2_ndf, ndf, bin_w, N,
                                         t_min, t_max, p_min, p_max,
                                         par_labels_theta, par_units_theta, 
                                         legend_data = legend,
                                         legend_fit=r'Fit: $\langle \theta(t) \rangle =  A_{\mathrm{B_z}}\cos(\omega_a t + \phi) + c$',
            ...
            # get residuals for later plots 
            residuals_theta[i_station] = cu.residuals(x, y, cu.Bz_only_phase, par)
        ...
        #     # Fit 
        #     par, par_e, pcov, chi2_ndf, ndf = cu.fit_and_chi2(x, y, y_e, cu.Bz_only_phase, p0_theta_truth[i_station])
        #     if (np.max(abs(par_e)) == np.Infinity ): raise Exception("\nOne of the fit parameters is infinity! Exiting...\n")
