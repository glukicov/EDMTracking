...
void TestOscBlinding(){
  ...
  for (int i(0); i<npoints; i++){
    yvals[i] = totOscWithOmegaA->Eval(xvals[i]) + preBlindingOsc->Eval(xvals[i]);
    double binE = sigmas[i] / sqrt(nentries);
    xerrs[i] = 0.0;
    yerrs[i] = binE;
  }

  TGraphErrors* preBlindingWithOmegaA = new TGraphErrors(npoints,xvals,yvals,xerrs,yerrs);
  ...
  preBlindingWithOmegaA->SetTitle("VBO + known EDM + blinded EDM + g-2");
  ...
  fitUnblindedWithOmegaA->SetParLimits(0,fakeAmp,fakeAmp+amp_blind);
  fitUnblindedWithOmegaA->SetParameter(1,freq);
  fitUnblindedWithOmegaA->SetParLimits(1,freq,freq);
  fitUnblindedWithOmegaA->SetParameter(2,fakePhase);
  fitUnblindedWithOmegaA->SetParLimits(2,fakePhase,fakePhase);
  // VBO
  ...
  fitUnblindedWithOmegaA->SetParLimits(4,omega_vbo,omega_vbo);
  fitUnblindedWithOmegaA->SetParameter(5,phi_vbo);
  fitUnblindedWithOmegaA->SetParLimits(5,phi_vbo,phi_vbo);
  // g-2
  ...
  fitUnblindedWithOmegaA->SetParLimits(7,freq,freq);
  fitUnblindedWithOmegaA->SetParameter(8,phi_a);
  fitUnblindedWithOmegaA->SetParLimits(8,phi_a,phi_a);
